...
type LoginPayload = { issuedAt: number; expiresAt: number; passwordChangedAt: number; nonce: string };
...
export function createLoginCookie(secure: boolean): string {
  const issuedAt = Date.now(); const payload: LoginPayload = { issuedAt, expiresAt: issuedAt + LOGIN_ABSOLUTE_EXPIRY_MS, passwordChangedAt: changedAt(), nonce: crypto.randomBytes(16).toString("base64url") };
...
export function isAuthenticatedCookie(cookieHeader: string | null): boolean {
  const value = cookieHeader?.split(/;\s*/).find((part) => part.startsWith(`${COOKIE}=`))?.slice(COOKIE.length + 1); if (!value) return false;
  const dot = value.lastIndexOf("."); if (dot < 1) return false; const data = value.slice(0, dot); const signature = value.slice(dot + 1);
  const actual = Buffer.from(signature); const expected = Buffer.from(sign(data)); if (actual.length !== expected.length || !crypto.timingSafeEqual(actual, expected)) return false;
  try { const payload = JSON.parse(Buffer.from(data, "base64url").toString()) as LoginPayload; return payload.expiresAt > Date.now() && payload.issuedAt <= Date.now() && payload.passwordChangedAt === changedAt(); } catch { return false; }
}
