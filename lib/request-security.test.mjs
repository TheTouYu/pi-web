...
test("allows wildcard-configured hostnames like trycloudflare tunnels", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const tunnel = new Request("https://shoes-era-analyst-erik.trycloudflare.com/api/test", {
    method: "POST",
    headers: {
      host: "shoes-era-analyst-erik.trycloudflare.com",
      origin: "https://shoes-era-analyst-erik.trycloudflare.com",
      "sec-fetch-site": "same-origin",
    },
  });
  assert.equal(isApiRequestAllowed(tunnel, ["*.trycloudflare.com"]), true);
  const notMatching = new Request("https://shoes.example.com/api/test", {
    method: "POST",
    headers: {
      host: "shoes.example.com",
      origin: "https://shoes.example.com",
      "sec-fetch-site": "same-origin",
    },
  });
  assert.equal(isApiRequestAllowed(notMatching, ["*.trycloudflare.com"]), false);
});
