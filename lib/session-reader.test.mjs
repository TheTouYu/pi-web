...
test("reuses unchanged session metadata and reparses only changed files", () => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-session-list-test-"));
  const sessionsDir = join(agentDir, "sessions", "project");
  mkdirSync(sessionsDir, { recursive: true });
  const unchanged = join(sessionsDir, "unchanged.jsonl");
  const changed = join(sessionsDir, "changed.jsonl");
  const session = (id, text) => [
    JSON.stringify({ type: "session", version: 3, id, timestamp: "2026-01-01T00:00:00.000Z", cwd: agentDir }),
    JSON.stringify(userEntry(`${id}-u1`, null, text)),
  ].join("\n") + "\n";
  writeFileSync(unchanged, session("unchanged", "keep cached"));
  writeFileSync(changed, session("changed", "refresh me"));

  try {
    const script = `
      const { createJiti } = require("jiti");
      (async () => {
        const module = await createJiti(process.cwd()).import(${JSON.stringify(join(import.meta.dirname, "session-reader.ts"))});
        await module.listAllSessions();
        const before = globalThis.__piSessionInfoCache;
        const unchangedEntry = before.get(${JSON.stringify(unchanged)});
        const changedEntry = before.get(${JSON.stringify(changed)});
        require("node:fs").appendFileSync(${JSON.stringify(changed)}, ${JSON.stringify(JSON.stringify(assistantEntry("changed-a1", "changed-u1", "done", "2026-01-01T00:00:01.000Z")) + "\n")});
        module.invalidateSessionListCache();
        const sessions = await module.listAllSessions();
        const after = globalThis.__piSessionInfoCache;
        console.log(JSON.stringify({
          unchangedReused: after.get(${JSON.stringify(unchanged)}) === unchangedEntry,
          changedReparsed: after.get(${JSON.stringify(changed)}) !== changedEntry,
          changedCount: sessions.find((session) => session.id === "changed").messageCount,
          awaitingReplyAt: sessions.find((session) => session.id === "changed").awaitingReplyAt,
        }));
    ...
    assert.deepEqual(JSON.parse(result.stdout.trim()), {
      ...
      changedReparsed: true,
      changedCount: 2,
      awaitingReplyAt: "2026-01-01T00:00:01.000Z",
    });
  } finally {
